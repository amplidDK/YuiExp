...
@dp.message(Text(text=['Да', 'Давай', 'Сыграем', 'Играть', 'Погнали', 'Игра'], ignore_case=True))
...
@dp.message(Text(text=['Нет', 'Не', 'Не хочу', 'Не буду', 'Отстань'], ignore_case=True))
...
async def process_number_determination(message: Message):
    if users[message.from_user.id]['in_game']:
        ...
        if users[message.from_user.id]['attempts'] == 0:
            await message.answer('К сожалению, у вас больше не осталось попыток. Вы проиграли :(\n\n'
...
@dp.message()
async def process_other_answers_determination(message: Message):
    if users[message.from_user.id]['in_game']:
        await message.answer('Мы же сейчас с вами играем. Присылайте, пожалуйста, числа от 1 до 100')
    else:
        await message.answer('Я довольно глуп, кожанный ты мешок. Давайте просто сыграем в игру?')
